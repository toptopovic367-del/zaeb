...
def process_city(message):
    try:
        ...
        if len(city) < 2:
            msg = bot.send_message(message.chat.id, '❌ Название города должно быть не короче 2 символов!\nПопробуй еще раз:')
            bot.register_next_step_handler(msg, process_city)
            return
